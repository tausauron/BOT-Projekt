...
public class RaumAlgorithmus
{
	...
	private void teileRaumVeranstaltungZu(ArrayList<ArrayList<Kurse>> eingeteilteKurseListe)
	{
		...
		System.out.println("----------------------------------------------------------------Zuordnung ende-------------------------------------------");
	}

	/**
	 * Diese Methode ist der Algorithmus. Hier wird der Passende Raum im bezug auf
	 * die Teilnehmergroesse des kurses zugeordnet. Ein Raum kann nicht doppelt
	...
	private void zuordnungErstellen(ArrayList<Kurse> slotKursListe, List<Raum> raumListe2)
	{
		...
		for (Kurse kurs : slotKursListe)
		{
			...
			if (bereitsZugeordneteRaeume.size() > raumListe2.size())	// Wenn bei aktuellem Kurs, die bereitsZugeordneteRaeume liste = raumliste groß
			{
				System.out.println("Kurs wird anders gespeichert");
				kursGehoertInNichtZuOrdbareListe(kurs,"Kein Raum mehr verfuegbar", true);
			} else
			{
				for (Raum raum : raumListe2)
				{
					...
					if (!bereitsZugeordneteRaeume.contains(raum)) // wenn raum nicht schon zugeteilt
					{
						...
						{
							kursGehoertInNichtZuOrdbareListe(kurs,"Fehlende Kapazitaet", true);
							System.out.println("Raum: " + raum.getName() + " keine Kapazitaet!! " + raum.getKapazitaet() + " VS "+ kurs.getKursTeilnehmer().size());
						}
	 ...
	 * Standart ist eine Aufsteigende Sortierung, deshalb wird die liste reversed. 
	 * 
	 * @param slotKursListe
	 */
